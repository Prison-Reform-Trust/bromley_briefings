...
def generate_html(fig: go.Figure) -> None:
    ...
    output_path = os.path.join(
        config['viz']['outPath'],
        "state_of_our_prisons/private_prisons.html")
    fig_html = fig.write_html(
        output_path,
        full_html=True,
...
def main() -> go.Figure:
    ...
    data_path = os.path.join(
        ...
        "state_of_our_prisons/private_prisons.csv")
    df = utils.load_data(data_path)
    fig = create_chart(df)
    # generate_html(fig)
    py.plot(fig, filename="private_prisons")
    return fig
